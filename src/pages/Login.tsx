      ...
      .catch(err => {
        if (err.response.status === 404) {
          setInvaliduserMessage(
            'Email is not registered, do you want to signup?'
          );
        }
        if (err.response.status === 401) {
          setInvaliduserMessage('Invalid Password!!');
        }
    ...
    <div className='max-w-screen-xl px-4 py-16 mx-auto sm:px-6 lg:px-8'>
      <div className='max-w-lg mx-auto'>
        <h1 className='text-2xl font-bold text-center text-indigo-600 sm:text-3xl'>
          Welcome back to Apnidukaan
        </h1>

        <form className='p-8 mt-6 mb-0 space-y-4 rounded-lg shadow-2xl'>
          <p className='text-lg font-medium'>Sign in to your account</p>
          <div className='text-red-400 '>{invaliduserMessage}</div>
